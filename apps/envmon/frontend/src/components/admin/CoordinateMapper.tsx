  ...
  const columns: Column<any>[] = [
    {
      ...
      render: (p) => (
        <>
          ...
          <span style={{ color: 'var(--text-3)', marginLeft: 6, fontSize: 12 }}>{p.plant_name}</span>
        </>
      )
    },
    {
      ...
      render: (p) => (
        <input
          ...
          style={{ width: 130, padding: '5px 8px', fontSize: 13, border: '1px solid var(--line-2)', borderRadius: 4, background: 'var(--surface-1)', color: 'var(--text-1)' }}
        />
      )
    },
    {
    ...
    },
    {
      ...
      render: (p) => savedFlags[p.plant_id] ? (
        <span style={{ fontSize: 12, color: 'var(--status-ok)', fontFamily: 'var(--font-mono)' }}>{t('envmon.admin.geo.saved')}</span>
      ) : (
        <Button
          variant="primary"
          size="sm"
