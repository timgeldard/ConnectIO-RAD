...
def test_fetch_orders_list_returns_coerced_rows(monkeypatch):
    ...
    assert len(result) == 1
    assert result[0]["process_order_id"] == _ORDER_ROW["process_order_id"]
...
def test_fetch_orders_list_with_plant_id_sends_params(monkeypatch):
    ...
    asyncio.run(dal.fetch_orders_list("token", plant_id=plant))
    # Now expects 2 params: limit and plant_id
    assert len(captured[0]) == 2
    assert any(p["name"] == "plant_id" and p["value"] == plant for p in captured[0])
...
def test_fetch_orders_list_multiple_orders_all_coerced(monkeypatch):
    row2 = test_data.generate_order_row({
        "process_order_id": "007006276689",
        "status": "completed",
        "actual_qty": "0.0",
    ...
    assert ids == {_ORDER_ROW["process_order_id"], "007006276689"}
    po2 = next(r for r in result if r["process_order_id"] == "007006276689")
...
def test_fetch_orders_list_preserves_non_coerced_fields(monkeypatch):
    ...
    result = asyncio.run(dal.fetch_orders_list("token"))
    row = result[0]
    assert row["material_name"] == _ORDER_ROW["material_name"]
    assert row["status"] == _ORDER_ROW["status"]
    assert row["qty_uom"] == _ORDER_ROW["qty_uom"]
    assert row["plant_id"] == _ORDER_ROW["plant_id"]
