...
import { PourLineFilter, PourKpiCards } from './PourAnalytics'
...
export function OrderList({ onOpen, lineFilter = 'ALL', setLineFilter = () => {} }: OrderListProps) {
  ...
  const filtered = useMemo(() => {
    ...
    const sorted = [...list].sort((a, b) => {
      ...
      switch (sortBy.key) {
        ...
        case 'qty': av = a.actualQty; bv = b.actualQty; break
    ...
    return sorted
  }, [search, statusFilter, productFilter, sortBy, lineFilter])

  const totalPages = Math.max(1, Math.ceil(filtered.length / PAGE_SIZE))
  const pageOrders = filtered.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE)

  useEffect(() => { setPage(1) }, [search, statusFilter, productFilter, dateRange, lineFilter])
  ...
  const totalRunning = (ORDERS as any[]).filter(o => o.status === 'running').length
  const onHold = (ORDERS as any[]).filter(o => o.status === 'onhold').length
  const avgYield = (() => {
    const ys = (ORDERS as any[]).filter(o => o.yieldPct != null).map(o => o.yieldPct as number)
    return (ys.reduce((a, b) => a + b, 0) / ys.length).toFixed(1)
  })()
  const ordersThisMonth = 287
    ...
    <>
      ...
      <div className="kpis">
        <div className="kpi">
          ...
          <div className="kpi-value">{totalRunning}<span className="kpi-delta neutral">{t.kpiActiveSub}</span></div>
          <Sparkline data={[2, 4, 3, 5, 4, 6, 5, 7, 6, 8, 5, 4]} color="var(--sage)" />
        </div>
        <div className="kpi">
          <div className="kpi-label">{t.kpiOrders30}</div>
          <div className="kpi-value">{ordersThisMonth}<span className="kpi-delta up">+12.4%</span></div>
          <Sparkline data={[12, 18, 15, 22, 19, 24, 28, 25, 30, 27, 29, 33]} color="var(--valentia-slate)" />
        </div>
        <div className="kpi">
          <div className="kpi-label">{t.kpiYield}</div>
          <div className="kpi-value">{avgYield}<span className="kpi-delta up">{t.kpiYieldDelta}</span></div>
        ...
        <div className="kpi">
          ...
          <div className="kpi-value">{onHold + 3}<span className="kpi-delta down">{t.kpiHoldSub}</span></div>
          <Sparkline data={[3, 2, 4, 2, 3, 1, 2, 3, 4, 5, 3, 2]} color="var(--sunset)" />
        </div>
      </div>

      <PourKpiCards lineFilter={lineFilter} />
      ...
      <div className="toolbar">
        ...
        <button className="date-pick">
          {I.calendar}<span>Apr 01</span><span className="sep">→</span><span>Apr 26, 2026</span>
        </button>

        <button className="chip">
          {I.filter}<span>{t.productLabel} · {productFilter || t.productAll}</span>
        </button>
              ...
              return (
                <tr key={o.id} className={isSel ? 'selected' : ''} onClick={() => onOpen(o)}>
                  ...
                  <td>
                    <div className="cell-product">
                      {o.line}
                      <span className="sku">{t.shift} {o.shift} · {o.operator}</span>
                    </div>
                  </td>
                  <td className="col-qty">
                    <div className="cell-num">
                      {fmt.num(o.actualQty)}<span className="unit">kg</span>
                  ...
                  <td className="col-date">
                    <div className="cell-date">
                      {fmt.shortDate(o.start)}, {fmt.time(o.start)}
                      <span className="time">{fmt.duration(o.durationH)}{o.status === 'running' ? ` · ${t.runningSuffix}` : ''}</span>
                    </div>
                  </td>
