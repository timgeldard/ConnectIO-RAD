...
def test_fetch_chart_data_page_generates_expected_sql(monkeypatch):
    ...
    asyncio.run(
        spc_charts_dal.fetch_chart_data_page(
            "token", mat_id, mic, "Viscosity", plant, "2026-01-01", "2026-01-31",
            "operation_id", cursor=spc_charts_dal.encode_chart_cursor(1704067200, b_id, "S1", lot, "OP1"), limit=100,
        )
    )
...
def test_build_chart_filters_rejects_invalid_stratify_key():
    ...
    with pytest.raises(ValueError, match="stratify_by must be one of"):
        spc_charts_dal._build_chart_filters(mat_id, mic, None, None, None, None, "bad_column")
...
def test_fetch_spec_drift_summary_filters_by_operation_id(monkeypatch):
    ...
    result = asyncio.run(spc_charts_dal.fetch_spec_drift_summary("token", mat_id, mic, plant, "2026-01-01", "2026-01-31", operation_id="OP-10"))
...
def test_fetch_spec_drift_summary_falls_back_when_summary_view_missing(monkeypatch):
    ...
    result = asyncio.run(spc_charts_dal.fetch_spec_drift_summary("token", mat_id, mic, None, None, None))
...
def test_fetch_normality_summary_maps_non_normal_profile(monkeypatch):
    ...
    result = asyncio.run(spc_charts_dal.fetch_normality_summary("token", mat_id, mic, plant, "2026-01-01", "2026-01-31", operation_id="OP-10"))
...
def test_fetch_control_limits_queries_governed_metrics(monkeypatch):
    ...
    result = asyncio.run(spc_charts_dal.fetch_control_limits("token", mat_id, mic, plant, "2026-01-01", "2026-01-31", operation_id="OP-10"))
...
def test_fetch_p_chart_data_uses_attribute_subgroup_mv(monkeypatch):
    ...
    async def fake_run_sql_async(_token, query, params=None, **_kwargs):
        captured["query"] = query
        captured["params"] = params or []
        return [{"batch_id": b_id, "batch_date": "2026-01-01", "n_inspected": "10", "n_nonconforming": "2", "p_value": "0.2"}]
    monkeypatch.setattr(spc_charts_dal, "run_sql_async", fake_run_sql_async)

    rows = asyncio.run(spc_charts_dal.fetch_p_chart_data("token", mat_id, mic, "Defects", plant, "2026-01-01", "2026-01-31", operation_id="OP-10"))
...
def test_fetch_count_chart_data_uses_attribute_subgroup_mv(monkeypatch):
    ...
    async def fake_run_sql_async(_token, query, params=None, **_kwargs):
        captured["query"] = query
        captured["params"] = params or []
        return [{"batch_id": b_id, "batch_date": "2026-01-01", "n_inspected": "12", "defect_count": "3"}]
    monkeypatch.setattr(spc_charts_dal, "run_sql_async", fake_run_sql_async)

    rows = asyncio.run(spc_charts_dal.fetch_count_chart_data("token", mat_id, mic, "Defects", plant, "2026-01-01", "2026-01-31", "u", operation_id="OP-10"))
...
def test_fetch_locked_limits_prefers_unified_mic_key(monkeypatch):
    ...
    result = asyncio.run(locked_limits_dal.fetch_locked_limits("token", mat_id, mic, plant, "imr", operation_id="OP-10", unified_mic_key="U-1"))
...
async def test_fetch_chart_data_values(monkeypatch):
    ...
    monkeypatch.setattr(spc_charts_dal, "run_sql_async", fake_run_sql_async)
    res = await spc_charts_dal.fetch_chart_data_values("token", mat_id, mic, "NM1", None, None, None)
...
async def test_delete_locked_limits(monkeypatch):
    ...
    mic = test_data.mic_id()
    plant = test_data.PLANTS[0]
    res = await locked_limits_dal.delete_locked_limits("token", mat_id, mic, plant, "imr")
...
async def test_fetch_data_quality_summary(monkeypatch):
    ...
    res = await spc_charts_dal.fetch_data_quality_summary("token", mat_id, mic, plant, None, None)
