...
export default function CapabilityHistogram({ spc }: CapabilityHistogramProps) {
  const option = useMemo(() => {
    ...
    if (xBar == null || !sigmaOverall || sigmaOverall <= 0) return null
    ...
    return {
      ...
      xAxis: {
        ...
        min: curveMin,
        max: curveMax,
        axisLabel: { fontSize: 9, color: '#6b7280', formatter: (v: number) => v.toFixed(3) },
        axisTick: { show: false },
      ...
      tooltip: {
        ...
        formatter: (params: EventParamLike) => {
          ...
          const value = Array.isArray(params.value) ? params.value[1] : null
          return `Normal curve: ${typeof value === 'number' ? value.toFixed(3) : '—'}`
        },
      },
      series: [
        {
          ...
          barWidth: barWidthPx,
          itemStyle: {
            color: (params: EventParamLike) => barData[params.dataIndex]?.inSpec
              ? 'rgba(27,58,75,0.65)'
              : 'rgba(239,68,68,0.65)',
          },
          markLine: {
