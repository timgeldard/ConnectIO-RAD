...
async def fetch_recall_readiness(token: str, material_id: str, batch_id: str) -> dict:
    ...
    header_rows, countries_rows, customers_rows, deliveries_rows, exposure_rows, events_rows = await asyncio.gather(
        run_sql_async(token, header_query, params),
        run_sql_async(token, countries_query, params),
        run_sql_async(token, customers_query, params),
        run_sql_async(token, deliveries_query, params),
        run_sql_async(token, exposure_query, params),
        run_sql_async(token, events_query, params),
    )
...
_BATCH_HEADER_SELECT = f"""
    ...
    LEFT JOIN {tbl('gold_batch_summary_v')} bs
...
async def fetch_batch_header(token: str, material_id: str, batch_id: str) -> Optional[dict]:
    query = _BATCH_HEADER_CTE + _BATCH_HEADER_SELECT
    rows = await run_sql_async(
        token,
        query,
        [sql_param("mat", material_id), sql_param("bat", batch_id)],
    )
    return rows[0] if rows else None
...
async def fetch_coa(token: str, material_id: str, batch_id: str) -> dict:
    ...
    results_query = f"""
        ...
        FROM {tbl('gold_batch_coa_results_v')}
        WHERE batch_id = :bat
        ORDER BY mic_code
    """
    ...
    header_rows, result_rows = await asyncio.gather(
        run_sql_async(token, header_query, params),
        run_sql_async(token, results_query, [sql_param("bat", batch_id)]),
    )
...
async def fetch_quality(token: str, material_id: str, batch_id: str) -> dict:
    ...
    lots_query = f"""
        ...
        FROM {tbl('gold_batch_quality_lot_v')}
        WHERE MATERIAL_ID = :mat AND BATCH_ID = :bat
    ...
    summary_query = f"""
        ...
        FROM {tbl('gold_batch_quality_summary_v')}
        WHERE MATERIAL_ID = :mat AND BATCH_ID = :bat
    """

    header_rows, lot_rows, result_rows, summary_rows = await asyncio.gather(
        run_sql_async(token, header_query, params),
        run_sql_async(token, lots_query, params),
        run_sql_async(token, results_query, params),
        run_sql_async(token, summary_query, params),
    )
...
async def fetch_production_history(token: str, material_id: str, batch_id: str, limit: int = 24) -> dict:
    ...
    prod_query = f"""
        ...
        FROM {tbl('gold_batch_production_history_v')}
    ...
    header_rows, prod_rows = await asyncio.gather(
        ...
        run_sql_async(
            token,
            prod_query,
            [sql_param("mat", material_id), sql_param("lim", str(limit))],
        ),
    )
...
async def fetch_batch_compare(token: str, material_id: str, batch_id: str, limit: int = 24) -> dict:
    params_mat = [sql_param("mat", material_id), sql_param("lim", str(limit))]
    header_params = [sql_param("mat", material_id), sql_param("bat", batch_id)]
    header_query = _BATCH_HEADER_CTE + _BATCH_HEADER_SELECT
    compare_query = f"""
        WITH prod AS (
        ...
        FROM prod p
        LEFT JOIN {tbl('gold_batch_quality_summary_v')} q
          ON q.MATERIAL_ID = :mat AND q.BATCH_ID = p.BATCH_ID
        ORDER BY p.POSTING_DATE DESC
    """

    header_rows, compare_rows = await asyncio.gather(
        run_sql_async(token, header_query, header_params),
        run_sql_async(token, compare_query, params_mat),
    )
...
async def fetch_bottom_up(token: str, material_id: str, batch_id: str, max_levels: int = 4) -> dict:
    params = [
        sql_param("mat", material_id),
        sql_param("bat", batch_id),
        sql_param("max_levels", str(max(1, max_levels))),
    ]
    header_query = _BATCH_HEADER_CTE + _BATCH_HEADER_SELECT
    lineage_query = f"""
        ...
        LEFT JOIN {tbl('gold_supplier')} s
          ON s.SUPPLIER_ID = a.supplier_id
        ORDER BY a.level, a.material_id
    """

    header_rows, lineage_rows = await asyncio.gather(
        run_sql_async(token, header_query, [sql_param("mat", material_id), sql_param("bat", batch_id)]),
        run_sql_async(token, lineage_query, params),
    )
...
async def fetch_top_down(token: str, material_id: str, batch_id: str, max_levels: int = 4) -> dict:
    ...
    # Deliveries / customers / countries reuse recall-readiness helper SQL
    del_params = [sql_param("mat", material_id), sql_param("bat", batch_id)]
    countries_query = f"""
        WITH per_delivery AS (
          SELECT DISTINCT DELIVERY, COUNTRY_ID, COUNTRY_NAME, ABS_QUANTITY
          FROM {tbl('gold_batch_delivery_v')}
    ...
    header_rows, lineage_rows, countries_rows, customers_rows, deliveries_rows = await asyncio.gather(
        run_sql_async(token, header_query, del_params),
        run_sql_async(token, lineage_query, params),
        run_sql_async(token, countries_query, del_params),
        run_sql_async(token, customers_query, del_params),
        run_sql_async(token, deliveries_query, del_params),
    )
...
async def fetch_supplier_risk(token: str, material_id: str, batch_id: str) -> dict:
    ...
    suppliers_query = f"""
        ...
        LEFT JOIN {tbl('gold_supplier')} s ON s.SUPPLIER_ID = a.supplier_id
        LEFT JOIN {tbl('gold_material')} m
    ...
    header_rows, supplier_rows = await asyncio.gather(
        run_sql_async(token, header_query, del_params),
        run_sql_async(token, suppliers_query, del_params),
    )
