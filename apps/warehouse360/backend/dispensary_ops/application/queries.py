...
from backend.dispensary_ops.domain.task_status import is_urgent, normalize_task_status
...
async def list_dispensary_tasks(token: str, plant_id: Optional[str] = None) -> list[dict]:
    ...
    and ``is_urgent`` (bool) derived from the raw ``status`` and ``mins_to_start``
    fields exposed by ``wh360_dispensary_tasks_v``.
    """
