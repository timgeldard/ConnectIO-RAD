...
async def list_kpis(token: str, plant_id: Optional[str] = None) -> list[dict]:
    ...
    Currently wh360_kpi_snapshot_v returns one wide row per plant.
    Health classification is handled by the frontend components until the
    view is unpivoted to per-KPI rows.
    """
