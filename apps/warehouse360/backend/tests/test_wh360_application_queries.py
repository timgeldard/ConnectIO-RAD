...
async def test_dispensary_queries_normalize_plant_scope(monkeypatch):
    ...
    result = await dispensary_queries.list_dispensary_tasks("token", f" {plant} ")
...
async def test_control_tower_queries_enrichment(monkeypatch):
    ...
    result = await control_tower_queries.list_kpis("token", plant)
...
async def test_order_fulfillment_list_queries_enrichment(monkeypatch):
    ...
    plant = test_data.PLANTS[2]
    po_id = test_data.process_order()

    async def fake_deliveries(token, plant_id=None):
        return [{"delivery_id": "8000123456", "delivery_status": "SHIPPED"}]

    async def fake_orders(token, plant_id=None):
        return [{"order_id": po_id, "order_status": "REL"}]
    ...
    deliveries = await fulfillment_queries.list_deliveries("token", plant)
    assert deliveries[0]["delivery_status_normalized"] == "SHIPPED"
    assert deliveries[0]["is_active"] is False

    orders = await fulfillment_queries.list_process_orders("token", plant)
...
async def test_order_fulfillment_detail_queries_delegate(monkeypatch):
    ...
    assert await fulfillment_queries.get_delivery_detail("token", del_id) == {"delivery": {"delivery_id": del_id}}
    assert await fulfillment_queries.get_process_order_detail("token", po_id) == {"order": {"order_id": po_id}}
    assert calls == [("delivery", "token", del_id), ("order", "token", po_id)]
