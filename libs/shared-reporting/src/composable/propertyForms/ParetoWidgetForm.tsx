...
import type { ParetoChartWidgetProps } from '../../widgets/widgetProps'
import { paretoChartWidgetPropsSchema } from '../../widgets/widgetProps'
...
/** Props for the ParetoWidgetForm component. */
export interface ParetoWidgetFormProps extends PropertyFormProps<ParetoChartWidgetProps> {
...
/**
 * Property form for configured Pareto Chart widgets.
 * Supports static items and live data binding.
 * 
 * @param props - Component properties
 * @returns React element
 */
export function ParetoWidgetForm({
  props,
  onChange,
  data,
  onDataChange,
  queryRegistry = {},
  dashboardParams = {},
}: ParetoWidgetFormProps) {
  const [activeTab, setActiveTab] = useState<'static' | 'data'>(data ? 'data' : 'static')

  const handleChange = (field: keyof ParetoChartWidgetProps, value: any) => {
    const newProps = { ...props, [field]: value === '' ? undefined : value }
    const result = paretoChartWidgetPropsSchema.safeParse(newProps)
    if (result.success) {
        ...
        <div style={sectionStyle}>
          ...
          {data && (
            <div style={advancedMappingStyle}>
              <div style={subheadingStyle}>Pareto Items Configuration</div>
              <label style={labelStyle}>
                <span>Label Field (in array)</span>
                <input
                  type="text"
                  value={(data.mapping?.items as any)?.config?.labelKey ?? 'label'}
                  onChange={(e) => handleMappingChange('items', {
                    path: (data.mapping?.items as any)?.path ?? '',
                    transform: 'paretoItems',
                    config: { ...((data.mapping?.items as any)?.config ?? {}), labelKey: e.target.value }
                  })}
                  style={inputStyle}
                />
              </label>
              <label style={labelStyle}>
                ...
                <input
                  type="text"
                  value={(data.mapping?.items as any)?.config?.valueKey ?? 'value'}
                  onChange={(e) => handleMappingChange('items', {
                    path: (data.mapping?.items as any)?.path ?? '',
                    transform: 'paretoItems',
                    config: { ...((data.mapping?.items as any)?.config ?? {}), valueKey: e.target.value }
                  })}
                  style={inputStyle}
                />
              </label>
